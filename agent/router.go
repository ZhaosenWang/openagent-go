...
type Router interface {
	...
	Route(ctx context.Context, input openagent.Message, agents []AgentInfo) (RouteResult, error)
	...
	CanHandoff(ctx context.Context, entry HandoffEntry, chain []HandoffEntry, session openagent.Session) error
}
...
func (FirstAgentRouter) Route(_ context.Context, _ openagent.Message, agents []AgentInfo) (RouteResult, error) {
	if len(agents) == 0 {
...
func (FirstAgentRouter) CanHandoff(_ context.Context, _ HandoffEntry, _ []HandoffEntry, _ openagent.Session) error {
...
func NewLLMRouter(model openagent.Model) *LLMRouter {
	return &LLMRouter{model: model}
}

func (r *LLMRouter) Route(ctx context.Context, input openagent.Message, agents []AgentInfo) (RouteResult, error) {
	if len(agents) == 0 {
		return RouteResult{}, fmt.Errorf("team has no agents")
	}
	if len(agents) == 1 {
	...
	resp, err := r.model.ChatCompletion(ctx, openagent.ChatCompletionRequest{
		Messages: []openagent.Message{
			{Role: openagent.RoleSystem, Content: prompt},
			{Role: openagent.RoleUser, Content: input.Content},
		},
		MaxTokens: 64,
	})
	if err != nil {
...
func (r *LLMRouter) CanHandoff(_ context.Context, _ HandoffEntry, _ []HandoffEntry, _ openagent.Session) error {
	return nil
}
