// Package crypto provides AES-128-ECB encryption/decryption for WeChat
// CDN media, plus the key encodings used by the ilinkai protocol
// (hex32 / base64(raw16) / base64(hex32)).
package crypto
...
func DecryptAESECB(ciphertext, key []byte) ([]byte, error) {
	...
	if len(ciphertext) == 0 || len(ciphertext)%aes.BlockSize != 0 {
		return nil, fmt.Errorf("ciphertext length %d is not a positive multiple of block size", len(ciphertext))
	}
...
func DecodeAESKey(encoded string) ([]byte, error) {
	...
	if len(decoded) == 2*aes.BlockSize && hexPattern.Match(decoded) {
...
func pkcs7Unpad(data []byte) ([]byte, error) {
	...
	// Strict: every padding byte must match (guards against corrupt data
	// silently truncating plaintext).
