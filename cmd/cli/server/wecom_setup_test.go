...
func TestClearWecomCredentialsPreservesOtherFields(t *testing.T) {
	...
	m := NewWecomManager(context.Background(), ".openagent/profile", nil, nil, kernel.Deps{})
...
func TestWecomQRCacheRoundTrip(t *testing.T) {
	...
	m := NewWecomManager(context.Background(), profiles, nil, nil, kernel.Deps{})
...
func TestWecomSetCredentialsRejectedWhileRegistering(t *testing.T) {
	m := NewWecomManager(context.Background(), filepath.Join(t.TempDir(), "profile"), nil, nil, kernel.Deps{})
	m.mu.Lock()
